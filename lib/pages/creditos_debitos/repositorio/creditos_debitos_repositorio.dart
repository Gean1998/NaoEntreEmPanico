...
class CreditosDebitosRepositorio {
  ...
  Future<double> obterCreditoRestante() async {
    try {
      ...
      final db = await BancoSQLite().obterConexao();
      final resultMap = await db.rawQuery(sql);
    ...
    try {
      ...
      switch (filtro) {
        ...
        case FILTRO_VALOR_ENUM.ANO:
          _filtro =
              " WHERE data BETWEEN '${ConverteData.database(DateTime(dataHoje.year, 01, 01))}' AND '${ConverteData.database(DateTime(dataHoje.year, 12, 31))}'";
          break;
        default:
          break;
      }
